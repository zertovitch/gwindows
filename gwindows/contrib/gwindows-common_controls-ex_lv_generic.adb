...
package body GWindows.Common_Controls.Ex_LV_Generic is
...
--   type Pointer_To_Nmcustomdraw_Type is access all Nmcustomdraw_Type;
   ...
   procedure Free is
      new Ada.Unchecked_Deallocation (Extended_Data_Array_Type,
   ...
   procedure Clear_Extended_Styles (Control : in Ex_List_View_Control_Type) is
      procedure Sendmessage
        (Hwnd   : Interfaces.C.long := Handle (Control);
         Umsg   : Interfaces.C.int  := Lvm_Setextendedlistviewstyle;
         Wparam : Integer           := 0;
   ...
   begin
      ...
      case Sort is
         when auto =>
            ...
            if Column = Control.Sort.Column and
              Control.Sort.Sort_Direction = Up then
   ...
   is
      ...
      Array_access : Extended_Data_Array_Access;
   begin
      Array_access := Address_Conversion.To_Pointer (Get_Lparam (Control,
                                                                 Index,
                                                                 0));
   ...
   begin
      Array_access := Address_Conversion.To_Pointer (Get_Lparam (Control,
                                                                 Index,
                                                                 0));
      Array_access (Sub_Index).More_Data := Data;
   ...
   begin
      Array_access := Address_Conversion.To_Pointer (Get_Lparam (Control,
                                                                 Index,
                                                                 0));
      return Array_access (Sub_Index).More_Data;
   end Get_Item_Data;
   ...
   procedure On_Create
     (Control : in out Ex_List_View_Control_Type) is
   begin
      Border (Control);
      Tab_Stop (Control);
   end On_Create;
   ...
   begin
      ...
      declare
         Texta : GString :=
           Text (Ex_List_View_Control_Type (Class_Ptr.all),
                 Indexa, Lparamsort.Column);
         Textb : GString :=
           Text (Ex_List_View_Control_Type (Class_Ptr.all),
                 Indexb, Lparamsort.Column);
      begin
   ...
   begin
      for I in 0 .. Item_Count (Window) - 1 loop
         Array_Access :=
           Address_Conversion.To_Pointer (Get_Lparam (Window, I, 0));
         Free (Array_Access);
      end loop;
